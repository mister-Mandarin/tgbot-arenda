...
async def nav_cal_handler_date(message: Message):
    calendar = SimpleCalendar(
        locale=await get_user_locale(message.from_user), cancel_btn=' ', today_btn='_', show_alerts=True
    )
    now = datetime.now()
    calendar.set_dates_range(now, now + relativedelta(months=3))
    await message.answer(
...
async def main() -> None:
    ...
    bot = Bot(os.environ['BOT_TOKEN'], default=DefaultBotProperties(parse_mode=ParseMode.HTML))   # works from aiogram v.3.7.0
